...
exports.run = async (client, message, args) => {
  ...
  if (!CEYetkili) return message.reply("Sistem ayarlanmamış!");
  if (!CELog) return message.reply("Sistem ayarlanmamış!");

  if (!message.member.roles.cache.has(CEYetkili))
    return message.reply(`> <@${message.author.id}> Ban Yetkin Olmadan Ban Sistemdeki Hiç Birşeyi Ayarlamassın.`);
  if (!CEKişi)
